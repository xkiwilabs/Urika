...
Project-level suggestions directory. Contains `initial.json` with the initial analytical approach suggestions generated by the project builder agent during project creation, and `pending.json` with queued experiment suggestions from advisor conversations (see [Advisor Chat and Instructions](07-advisor-and-instructions.md#persistent-memory)).
...
| `advisor-history.json` | Persistent advisor conversation history (append-only audit log) |
| `advisor-context.md` | Rolling research context summary loaded into every advisor call |
