...
| #120 | Advisor: nav-resilient + visible in banner + planner reads history | ~½ day | Three connected fixes: (1) Wrap `runner.run` in `asyncio.shield()` so navigating away doesn't cancel the in-flight advisor — the response always persists to `advisor-history.json`. (2) Write `<project>/.urika/.advisor.inflight` marker at start + remove at end; add to `_PROJECT_LEVEL_LOCKS` in `active_ops.py` so the running-ops banner shows an "advisor" chip while it's thinking. (3) Update `planning_agent_system.md` to read `projectbook/advisor-history.json` so per-turn planners see what the user has been discussing — currently only the meta-loop (auto mode) consumes advisor context. |
