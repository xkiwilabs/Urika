...
@pytest.mark.skip(
    reason=(
        "Superseded by dev/scripts/smoke-v04-e2e-*.sh (open/hybrid/private). "
        "This test mocks `urika run` but not `urika report`, so it spawned "
        "the Report Agent against the live API every pytest run. The "
        "modern Report Agent also emits narrative.md rather than the "
        "results-summary.md / key-findings.md filenames asserted below — "
        "those are now generated by core.labbook helpers, not the agent. "
        "The dev/scripts/ E2E suite covers this end-to-end with real "
