...
def test_report_post_when_already_running_redirects_to_log(report_client):
    """HTMX POST while a report is already running for this experiment
    ...
    exp_dir = proj / "experiments" / "exp-001"
    exp_dir.mkdir(parents=True, exist_ok=True)
    (exp_dir / ".report.lock").write_text(str(os.getpid()))
...
def test_report_post_when_already_running_returns_409_without_hx(report_client):
    ...
    assert body["log_url"] == "/projects/alpha/experiments/exp-001/log?type=report"
    assert body["type"] == "report"
    assert spawn_calls == []


def test_report_post_other_experiment_running_does_not_block(report_client):
    """A live report lock on a *different* experiment must NOT block
    a fresh report spawn for this experiment — locks are per-experiment."""
    import os
    ...
    (other / ".report.lock").write_text(str(os.getpid()))
