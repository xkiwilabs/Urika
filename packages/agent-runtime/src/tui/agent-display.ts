...
/**
 * Register additional agent colors at runtime.
 * Host systems can extend the default palette with their own agent roles.
 */
export function registerAgentColor(role: string, colorFn: (s: string) => string): void {
  AGENT_COLORS[role] = colorFn;
}
